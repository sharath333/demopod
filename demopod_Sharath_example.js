...
router.route('/demopods/:SPECID').get(function (req, res) {
    ...
    oracledb.getConnection(connectionProperties, function (err, connection) {
        ...
        connection.execute("Select a.demopod, b.specialization from demopods a, demopod_spec c, SPECIALIZATIONS b where a.podid = c.podid AND b.specid= c.specid AND b.specid = :SPECID", [req.params.SPECID], {
            outFormat: oracledb.OBJECT // Return the result as Object
        }, function (err, result) {
            if (err || result.rows.length < 1) {
                res.set('Content-Type', 'application/json');
    ...
    oracledb.getConnection(connectionProperties, function (err, connection) {
        ...
        connection.execute("select * from demopod_questions_random where podid = :PODID and rownum < 6", [req.params.PODID], {
			...
			for (var i=0;i<jsonResult.length;i++) {
				...
				connection.execute("SELECT PODANSWERID, ANSWER From DEMOPOD_ANSWERS WHERE PODQUESTIONID = :PODQUESTIONID AND PODID = :PODID", { PODQUESTIONID: { type: oracledb.NUMBER, dir : oracledb.BIND_IN, val : req.body.PODQUESTIONID },PODID: { type: oracledb.NUMBER, dir : oracledb.BIND_IN, val : req.body.PODID }}, {
					outFormat: oracledb.OBJECT // Return the result as Object
				}, function (err1, result1) {
					if (err1 || result1.rows.length < 1) {
						res.set('Content-Type', 'application/json');
						var status = err1 ? 500 : 404;
						res.status(status).send(JSON.stringify({
							status: status,
							message: err1 ? "Error getting the answer set" : "demopod id doesn't exist",
							detailed_message: err1 ? err1.message : ""
						}));
						doRelease(connection);
					}
					var ansList = JSON.stringify(result1.rows);
					for (var j=0; j<ansList.length;j++) {
						jsonResult[i]["PODANSWERID"+j] = ansList[j]["PODANSWERID"];
						jsonResult[i]["ANSWER"+j] = ansList[j]["ANSWER"];
					}
					// Release the connection
					doRelease(connection);
				});
			...
			res.contentType('application/json').status(200).send(jsonResult);
